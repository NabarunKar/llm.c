/*
GPT-2 Transformer Neural Net training loop with RoPE and RMSNorm.
Based on train_gpt2.cu with modifications:
1. RoPE (Rotary Position Embeddings) instead of absolute positional embeddings
...
#include "llmc/rmsnorm.cuh"  // Using RMSNorm instead of LayerNorm
...
// Include RoPE implementation
#include "llmc/rope.cuh"

// GPT-2 model definition with RoPE configuration
typedef struct {
    int max_seq_len;
    int vocab_size;
...
// Parameters - removed bias terms for RMSNorm (ln1b, ln2b, lnfb)
constexpr const int NUM_PARAMETER_TENSORS = 13;  // Was 16, now 13 (removed 3 bias terms)
typedef struct {
    floatX* wte;       // (V, C)
    floatX* wpe;       // (maxT, C) - kept for compatibility, but won't be used with RoPE
    floatX* ln1w;      // (L, C) - RMSNorm weight only
    floatX* qkvw;      // (L, 3*C, C)
    floatX* qkvb;      // (L, 3*C)
    floatX* attprojw;  // (L, C, C)
    floatX* attprojb;  // (L, C)
    floatX* ln2w;      // (L, C) - RMSNorm weight only
    floatX* fcw;       // (L, 4*C, C)
    floatX* fcb;       // (L, 4*C)
    floatX* fcprojw;   // (L, C, 4*C)
    floatX* fcprojb;   // (L, C)
    floatX* lnfw;      // (C) - RMSNorm weight only
} ParameterTensors;
...
// Activations - removed mean terms, kept only rstd for RMSNorm
constexpr int NUM_ACTIVATION_TENSORS = 18;  // Adjusted from 21
typedef struct {
    floatX* encoded;     // (B, T, C)
    floatX* ln1;         // (L, B, T, C)
    float* ln1_rstd;     // (L, B, T) - RMSNorm only needs rstd
    floatX* atty;        // (L, B, T, C)
#if ENABLE_CUDNN
    float* att;          // (L, B, NH, T)
#else
    ...
    floatX* ln2;         // (L, B, T, C)
    float* ln2_rstd;     // (L, B, T) - RMSNorm only needs rstd
    floatX* fch;         // (L, B, T, 4*C)
    floatX* fch_gelu;    // (L, B, T, 4*C)
    floatX* residual3;   // (L, B, T, C)
    floatX* lnf;         // (B, T, C)
    float* lnf_rstd;     // (B, T) - RMSNorm only needs rstd
    float* losses;       // (B, T)
    floatX* qkvr;        // (L, B, T, 3*C)
    floatX* output;      // (B, T, max(3C, max(NH*T, V)))
...
void fill_in_activation_sizes(const ActivationTensors* data, TensorSpec (&tensors)[NUM_ACTIVATION_TENSORS], size_t B, size_t T, GPT2Config config, int recompute) {
    ...
    tensors[2] = TENSOR_SPEC(data->ln1_rstd, L * B * T);  // Only rstd for RMSNorm
    ...
    tensors[7] = TENSOR_SPEC(data->ln2_rstd, L * B * T);  // Only rstd for RMSNorm
    ...
    tensors[12] = TENSOR_SPEC(data->lnf_rstd, B * T);  // Only rstd for RMSNorm
...
void gpt_build_from_descriptor(GPT2 *model, const char* descriptor) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
            ...
            if (l == 0 && (i == 2 || i == 7 || i == 12)) {  // ln1w, ln2w, lnfw
...
void gpt2_forward(GPT2 *model, const int* inputs, size_t B, size_t T) {
    ...
    // Encoder forward - if using RoPE, we don't use wpe
    if (model->config.use_rope) {
        encoder_forward(acts.encoded, model->inputs, params.wte, NULL, B, T, C, main_stream);
    } else {
        encoder_forward(acts.encoded, model->inputs, params.wte, params.wpe, B, T, C, main_stream);
    }

    // First RMSNorm (not fused)
    rmsnorm_forward(acts.ln1, acts.ln1_rstd, acts.encoded, params.ln1w, B, T, C, main_stream);
    ...
    for (int l = 0; l < L; l++) {
        ...
        if (model->config.use_rope) {
            rope_forward(l_qkvr, B, T, C, NH, model->config.rope_dim, model->config.rope_base, main_stream);
        }
        ...
        // Apply RoPE if enabled
        if (model->config.use_rope) {
            rope_forward(scratch, B, T, C, NH, model->config.rope_dim, model->config.rope_base, main_stream);
        }
        ...
        if(l+1 != L) {
            ...
            fused_residual_rmsnorm_forward(l_residual3, acts.lnf, acts.lnf_rstd, l_residual2, scratch, params.lnfw, B * T, C, main_stream);
        }
    }
...
void gpt2_backward_and_reduce(GPT2 *model, int* inputs, const int* targets, int grad_accum_steps, int micro_step) {
    ...
    for (int l = L-1; l >= 0; l--) {
        ...
        // Apply RoPE backward if enabled
        if (model->config.use_rope) {
            rope_backward(dl_bt4c, B, T, C, NH, model->config.rope_dim, model->config.rope_base, main_stream);
        }
#else
        floatX* l_att = acts.att + l * B * NH * T * T;
    ...
    // Encoder backward - handle wpe differently based on RoPE
    if (model->config.use_rope) {
    ...
    if(last_step) {
        ...
        // Gradient reduction for non-transformer parameters
        if (model->config.use_rope) {
            floatX* const pointers[] = {grads.wte, grads.lnfw};
            const size_t nelem[] = {Vp * C, C};
            multi_gpu_async_reduce_gradient(pointers, nelem, &multi_gpu_config, main_stream);
        } else {
            floatX* const pointers[] = {grads.wte, grads.wpe, grads.lnfw};
            const size_t nelem[] = {Vp * C, T * C, C};
            multi_gpu_async_reduce_gradient(pointers, nelem, &multi_gpu_config, main_stream);
        }
    }
...
ShardInfo gpt2_get_tensor_at_layer(const GPT2 *model, int layer_id, int param_tensor_id) {
    ...
    // Adjusted for 13 parameters (removed 3 bias terms): transformer block is now tensors 2-11
    if(2 <= param_tensor_id && param_tensor_id <= 11) {
...
float gpt2_calculate_grad_norm(GPT2 *model, MultiGpuConfig* multi_gpu_config) {
    ...
    if (multi_gpu_config->zero_stage == 1) {
        ...
        for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
            ...
            // Adjusted bounds for 13 parameters
            if((i < 2 || i > 11)) {
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, float grad_scale, int t,
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        ...
        // Adjusted indices for 13 parameters: wte(0), wpe(1), qkvw(3), attprojw(5), fcw(8), fcprojw(10)
        float wd = (i == 0 || i == 1 || i == 3 || i == 5 || i == 8 || i == 10) ? weight_decay : 0.0f;
...
void error_usage() {
    ...
    fprintf(stderr, "  -e <string> input .bin filename or descriptor\n");
    fprintf(stderr, "  -o <string> output log dir\n");
    fprintf(stderr, "  -n <int>    checkpoint every n steps\n");
    fprintf(stderr, "  -y <int>    resume from checkpoint step\n");
    fprintf(stderr, "  -b <int>    micro batch size B\n");
    fprintf(stderr, "  -t <int>    sequence length T\n");
    fprintf(stderr, "  -d <int>    total batch size (B*T)\n");
    fprintf(stderr, "  -l <float>  learning rate\n");
    fprintf(stderr, "  -lg <int>   log GPU utilization every n steps\n");
    fprintf(stderr, "  -u <int>    warmup iterations\n");
    fprintf(stderr, "  -q <float>  final learning rate fraction\n");
    fprintf(stderr, "  -c <float>  weight decay\n");
    fprintf(stderr, "  -x <int>    max steps\n");
    fprintf(stderr, "  -v <int>    validate every n steps\n");
    fprintf(stderr, "  -m <int>    max steps for validation\n");
    fprintf(stderr, "  -s <int>    sample every n steps\n");
    fprintf(stderr, "  -ge <int>   fuse gelu via cuBLASLt (0=none, 1=forward, 2=forward+backward)\n");
    fprintf(stderr, "  -g <int>    generation length\n");
    fprintf(stderr, "  -a <int>    overfit single batch (1=yes)\n");
    fprintf(stderr, "  -f <int>    override enable TF32 (1=yes)\n");
    fprintf(stderr, "  -w <int>    use master weights (1=yes)\n");
    fprintf(stderr, "  -z <int>    zero stage (0=off, 1=on)\n");
    fprintf(stderr, "  -r <int>    recompute (0=off, 1=on)\n");
    fprintf(stderr, "  -h <int>    HellaSwag eval (0=off, 1=on)\n");
    fprintf(stderr, "  -k <string> learning rate scheduler type\n");
    fprintf(stderr, "  -pi <string> NCCL init method (e.g. 'env', 'file', 'mpi')\n");
    fprintf(stderr, "  -pf <string> file system path for distributed training\n");
    fprintf(stderr, "  -ps <string> server IP for distributed training\n");
    fprintf(stderr, "  -pn <int>    number of processes for distributed training\n");
    fprintf(stderr, "  -pr <int>    process rank for distributed training\n");
    fprintf(stderr, "  -pg <int>    GPUs per node for distributed training\n");
    fprintf(stderr, "  -nk <int>    keep latest n checkpoints\n");
    fprintf(stderr, "  -nm <int>    major checkpoint every n steps\n");
    exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i+=2) {
        ...
        if (!(strlen(argv[i]) == 2 || strlen(argv[i]) == 3)) { error_usage(); }
    ...
    // Set RoPE dimension after model is built
    model.config.rope_dim = model.config.channels / model.config.num_heads;
    ...
    printf0("| use RoPE              | %-50s |\n", model.config.use_rope ? "yes" : "no");
    printf0("| rope_dim              | %-50d |\n", model.config.rope_dim);
    printf0("| rope_base             | %-50.1f |\n", model.config.rope_base);
    ...
    for (; step <= train_num_batches; step++) {
        ...
        if(log_gpu_every > 0 && (step + 1) % log_gpu_every == 0) {
            ...
            printf0("                  compute %2.1f%% | memory: %2.1f%% | fan: %2d%% | %4d MHz / %4d MHz | %3d W / %3d W | %d°C / %d°C | %s\n",
                    gpu_info.gpu_utilization, gpu_info.mem_utilization, gpu_info.fan, gpu_info.clock, gpu_info.max_clock, 
